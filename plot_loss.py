...
def plot_training_history(log_path="complex_check_wind_phase/wind_transfer_log.csv", save_path="learning_curve_wind_phase.png"):
    ...
    plt.plot(df['Epoch'], df['Train_Loss'], label='Train wSDR Loss', linewidth=2, color='royalblue')
    plt.plot(df['Epoch'], df['Val_Loss'], label='Validation wSDR Loss', linewidth=2, color='darkorange')
    ...
    print(f"Best Configuration: Epoch {best_epoch} (Val Loss: {df['Val_Loss'].iloc[best_val_idx]:.4f})")
